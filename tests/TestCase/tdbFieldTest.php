...
class tdbTableTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testCreateTableInvalidFields()
    {
        $this->setExpectedException('TextDb\Exception\InvalidArgumentException');

        $this->tdb->createTable("test", "fields");
    }

    public function testCreateTableTableAlreadyExists()
    {
        $this->setExpectedException('TextDb\Exception\InvalidArgumentException');

        $this->tdb->createTable("test", [["id", "id"], ["name", "string", 50]]);
        $this->tdb->createTable("test", [["id", "id"], ["email", "string", 100]]);
